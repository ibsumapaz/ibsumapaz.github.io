...
— ${r.credits.join(`
`)}`:"",c=`🎶 *Himno ${t}: ${e}*${s}
${a}${i}${n}${l}`;if(navigator.share)try{await navigator.share({title:`Himno ${t}: ${e}`,text:c});return}catch(u){if(u.name!=="AbortError")console.warn("Share API failed, fallback to clipboard",u);else return}try{await navigator.clipboard.writeText(c),$("Letra copiada al portapapeles")}catch{const p=document.createElement("textarea");p.value=c,p.style.position="fixed",p.style.opacity="0",document.body.appendChild(p),p.focus(),p.select();try{document.execCommand("copy"),$("Letra copiada al portapapeles")}catch{$("No se pudo copiar el texto")}document.body.removeChild(p)}}class q{constructor(t){this.container=t,this.currentDetailVariantIndex=0,this.renderedView={type:null,tab:null,category:null,playlistName:null,hymnNumber:null,mode:null},this.currentActiveModal=null,this.renderedAudioTrackUrl=null,this.isUserSeeking=!1}init(){this.renderSkeleton(),this.bindGlobalEvents(),this.initSidebar(),this.initHeader(),this.initNav(),o.subscribe(()=>{this.render()}),E.subscribe(()=>{this.updatePlayerUI()}),this.render()}renderSkeleton(){this.container.innerHTML=`
    ...
    `,(a=document.getElementById("btn-sidebar-home"))==null||a.addEventListener("click",()=>{o.setActiveTab(0),o.setSelectedHymn(null)}),t.querySelectorAll("[data-sidebar-tab]").forEach(n=>{n.addEventListener("click",()=>{const l=parseInt(n.getAttribute("data-sidebar-tab"),10);o.setActiveTab(l)})}),(s=document.getElementById("btn-sidebar-theme-toggle"))==null||s.addEventListener("click",()=>{o.toggleTheme()}),(i=document.getElementById("btn-sidebar-profile"))==null||i.addEventListener("click",()=>{o.openModal("PROFILE")}),this.updateSidebar()}updateSidebar(){const t=document.getElementById("app-sidebar");if(!t)return;const e=o.get(),a=e.userProfile,s=U(),i=a.name?a.name.trim().charAt(0).toUpperCase():"U",n=a.avatarColor||"#14C69B";t.querySelectorAll("[data-sidebar-tab]").forEach(v=>{const f=parseInt(v.getAttribute("data-sidebar-tab"),10),y=e.activeTab===f&&!e.selectedHymn;v.classList.toggle("active",y)});const l=document.getElementById("sidebar-count-all");l&&(l.textContent=e.hymnsList.length);const c=document.getElementById("sidebar-count-playlists");c&&(c.textContent=e.playlists.length);const u=document.getElementById("sidebar-count-favs");u&&(u.textContent=e.favorites.size);const p=document.getElementById("sidebar-theme-icon");p&&(p.innerHTML=s?d.moon:d.sun);const h=document.getElementById("sidebar-theme-label");h&&(h.textContent=s?"Modo Oscuro":"Modo Claro");const m=document.getElementById("sidebar-user-avatar");m&&(m.style.backgroundColor=n,a.profileImage?m.innerHTML=`<img src="${a.profileImage}" alt="Avatar">`:m.innerHTML=`<span id="sidebar-avatar-initial">${i}</span>`);const g=document.getElementById("sidebar-profile-name");g&&(g.textContent=a.name||"Mi Perfil")}initHeader(){var a,s,i,n;const t=document.getElementById("app-header");if(!t)return;t.innerHTML=`
    ...
    `).join(""),t.querySelectorAll(".nav-item").forEach(a=>{a.addEventListener("click",()=>{const s=parseInt(a.getAttribute("data-tab"),10);o.setActiveTab(s)})}),this.updateNav()}updateNav(){const t=document.getElementById("bottom-nav");if(!t)return;const e=o.get();t.querySelectorAll(".nav-item").forEach(a=>{const s=parseInt(a.getAttribute("data-tab"),10),i=e.activeTab===s&&!e.selectedHymn;a.classList.toggle("active",i)})}renderContent(){const t=document.getElementById("main-viewport");if(!t)return;const e=o.get();if(e.isLoading){this.renderedView.type!=="LOADING"&&(t.innerHTML=`
        ...
        `,this.renderedView={type:"LOADING"});return}if(e.selectedHymn){const a=e.selectedHymn.number;this.renderedView.type!=="DETAIL"||this.renderedView.hymnNumber!==a?(this.renderHymnDetail(t,e.selectedHymn),this.renderedView={type:"DETAIL",hymnNumber:a}):this.updateHymnDetailDynamicElements(t,e.selectedHymn);return}switch(e.activeTab){case 0:this.renderHymnsListTab(t);break;case 1:this.renderCategoriesTab(t);break;case 2:this.renderPlaylistsTab(t);break;case 3:this.renderFavoritesTab(t);break;case 4:this.renderInfoTab(t);break;default:this.renderHymnsListTab(t)}}renderHymnsListTab(t){var n,l;const e=o.get(),a=H(e.hymnsList,{activeTab:0,searchQuery:e.searchQuery,favorites:e.favorites}),s=a.filter(c=>c.mp3Urls&&c.mp3Urls.length>0);if(this.renderedView.type==="TAB_0"){const c=t.querySelector(".hymns-grid-container");c&&(c.innerHTML=this.buildHymnGridItems(a,e.searchQuery,e.favorites));const u=t.querySelector(".view-subheading");u&&(u.textContent=`${a.length} ${a.length===1?"himno disponible":"himnos disponibles"}`);const p=document.getElementById("btn-play-all-hymns");p&&(p.style.display=s.length>0?"inline-flex":"none");const h=document.getElementById("mobile-search-input");h&&h.value!==(e.searchQuery||"")&&(h.value=e.searchQuery||"");const m=document.getElementById("btn-clear-mobile-search");m&&m.classList.toggle("hidden",!e.searchQuery),this.bindHymnCardClicks();return}this.renderedView={type:"TAB_0"},t.innerHTML=`
    ...
    `,t.querySelectorAll(".chip-btn").forEach(l=>{l.addEventListener("click",()=>{const c=l.getAttribute("data-mode");o.setCategoryMode(c)})}),t.querySelectorAll(".category-card").forEach(l=>{l.addEventListener("click",()=>{const c=l.getAttribute("data-category");o.setSelectedCategory(c)})})}renderPlaylistsTab(t){var s,i,n,l,c;const e=o.get();if(e.selectedPlaylist){const u=e.selectedPlaylist,p=u.hymns.map(m=>e.hymnsList.find(g=>g.number===m)).filter(Boolean),h=p.filter(m=>m.mp3Urls&&m.mp3Urls.length>0);this.renderedView={type:"PL_DETAIL",playlistName:u.name},t.innerHTML=`
        <div class="view-content-wrapper fade-in">
          <div class="playlist-detail-header">
            ...
            <div style="flex: 1;">
              ...
              <span class="view-subheading">${p.length} himnos</span>
            </div>
            ${h.length>0?`
              ...
              ${p.map((m,g)=>`
                <div class="hymn-grid-item" data-number="${m.number}">
                  ...
                  <div class="playlist-hymn-actions" onclick="event.stopPropagation();">
                    ...
                    <button class="playlist-reorder-btn btn-move-down" data-num="${m.number}" ${g===p.length-1?"disabled":""} title="Bajar">
                      ${d.arrowDown}
                    </button>
    ...
    `,(n=document.getElementById("btn-create-playlist-header"))==null||n.addEventListener("click",()=>{o.openModal("CREATE_PLAYLIST")}),(l=document.getElementById("btn-create-playlist-empty"))==null||l.addEventListener("click",()=>{o.openModal("CREATE_PLAYLIST")}),(c=document.getElementById("fab-create-playlist"))==null||c.addEventListener("click",()=>{o.openModal("CREATE_PLAYLIST")}),t.querySelectorAll(".playlist-card").forEach(u=>{u.addEventListener("click",p=>{if(p.target.closest(".playlist-card-delete-btn"))return;const h=u.getAttribute("data-playlist"),m=a.find(g=>g.name===h);m&&o.setSelectedPlaylist(m)})}),t.querySelectorAll(".playlist-card-delete-btn").forEach(u=>{u.addEventListener("click",p=>{p.stopPropagation();const h=u.getAttribute("data-delete-playlist");o.openModal("DELETE_PLAYLIST",{name:h})})})}renderFavoritesTab(t){var i;const e=o.get(),a=H(e.hymnsList,{activeTab:3,searchQuery:e.searchQuery,favorites:e.favorites}),s=a.filter(n=>n.mp3Urls&&n.mp3Urls.length>0);if(this.renderedView.type==="FAVORITES"){if(e.favorites.size===0){t.innerHTML=`
        ...
        `;return}const n=t.querySelector(".hymns-grid-container");n&&(n.innerHTML=this.buildHymnGridItems(a,e.searchQuery,e.favorites));const l=t.querySelector(".view-subheading");l&&(l.textContent=`${a.length} himnos guardados`);const c=document.getElementById("btn-play-all-favs");c&&(c.style.display=s.length>0?"inline-flex":"none"),this.bindHymnCardClicks();return}if(this.renderedView={type:"FAVORITES"},e.favorites.size===0){t.innerHTML=`
        <div class="view-content-wrapper fade-in">
          <div class="empty-state">
            <div class="empty-state-icon">${d.heartOutline}</div>
            <div class="empty-state-title">No tienes himnos favoritos</div>
    ...
    `,(i=document.getElementById("btn-play-all-favs"))==null||i.addEventListener("click",()=>{const n=o.get(),c=H(n.hymnsList,{activeTab:3,searchQuery:n.searchQuery,favorites:n.favorites}).filter(u=>u.mp3Urls&&u.mp3Urls.length>0);c.length>0&&E.play(c[0],c)}),this.bindHymnCardClicks()}renderInfoTab(t){this.renderedView.type!=="INFO"&&(this.renderedView={type:"INFO"},t.innerHTML=`
      <div class="view-content-wrapper fade-in">
            ...
            <div class="detail-player-card">
              ...
              <div class="player-seekbar-row" id="detail-seekbar-row" style="display: ${l&&n.duration>0?"block":"none"};">
                ...
                <div class="player-times">
                  <span id="detail-time-current">${w(n.currentTime)}</span>
                  <span id="detail-time-duration">${w(n.duration)}</span>
                </div>
              </div>
    ...
    `,(p=document.getElementById("btn-back-detail"))==null||p.addEventListener("click",()=>{o.setSelectedHymn(null)}),(h=document.getElementById("btn-share-detail"))==null||h.addEventListener("click",()=>{D(e)}),(m=document.getElementById("btn-add-playlist-detail"))==null||m.addEventListener("click",()=>{o.openModal("ADD_TO_PLAYLIST",{hymn:e})}),(g=document.getElementById("btn-fav-detail"))==null||g.addEventListener("click",()=>{const y=o.toggleFavorite(e.number);$(y?"Añadido a favoritos":"Eliminado de favoritos")});const c=document.getElementById("btn-detail-play");c==null||c.addEventListener("click",()=>{const y=E.getState(),I=e.mp3Urls&&e.mp3Urls[this.currentDetailVariantIndex]||e.mp3Urls&&e.mp3Urls[0];I&&(y.currentUrl===I?y.isPlaying?E.pause():E.resume():E.play(e,i,I))}),(v=document.getElementById("btn-detail-prev"))==null||v.addEventListener("click",()=>{E.playPrevious()}),(f=document.getElementById("btn-detail-next"))==null||f.addEventListener("click",()=>{E.playNext()});const u=document.getElementById("detail-seekbar");u==null||u.addEventListener("input",y=>{E.seekTo(parseFloat(y.target.value))}),t.querySelectorAll(".tone-chip").forEach(y=>{y.addEventListener("click",()=>{const I=parseInt(y.getAttribute("data-variant-idx"),10),L=y.getAttribute("data-url");this.currentDetailVariantIndex=I,t.querySelectorAll(".tone-chip").forEach(T=>T.classList.remove("active")),y.classList.add("active");const k=E.getState();k.currentHymn&&k.currentHymn.number===e.number&&E.play(e,i,L)})})}updateHymnDetailDynamicElements(t,e){const s=o.get().favorites.has(e.number),i=document.getElementById("btn-fav-detail");i&&(i.classList.toggle("is-fav",s),i.innerHTML=s?d.heart:d.heartOutline,i.title=s?"Quitar de favoritos":"Añadir a favoritos");const n=E.getState(),l=n.currentHymn&&n.currentHymn.number===e.number,c=document.getElementById("btn-detail-play");c&&(c.innerHTML=l&&n.isPlaying?d.pause:d.play);const u=document.getElementById("detail-seekbar-row");u&&(u.style.display=l&&n.duration>0?"block":"none");const p=document.getElementById("detail-seekbar");p&&l&&(p.max=Math.floor(n.duration),this.isUserSeeking||(p.value=Math.floor(n.currentTime)));const h=document.getElementById("detail-time-current");h&&l&&(h.textContent=w(n.currentTime));const m=document.getElementById("detail-time-duration");m&&l&&(m.textContent=w(n.duration))}updatePlayerUI(){var y,I,L,k,T,M;const t=document.getElementById("bottom-player-host");if(!t)return;const e=E.getState(),a=e.currentHymn;if(!a||!e.currentUrl){this.renderedAudioTrackUrl!==null&&(t.innerHTML="",this.renderedAudioTrackUrl=null);return}const s=o.get(),i=s.favorites.has(a.number),n=e.duration>0?e.currentTime/e.duration*100:0,l=z(e.currentUrl);if(this.renderedAudioTrackUrl!==e.currentUrl){this.renderedAudioTrackUrl=e.currentUrl,t.innerHTML=`
        <div class="web-bottom-player">
          ...
          <div class="player-center-controls">
            ...
            <div class="player-timeline-bar">
              <span class="player-time-current" id="player-time-current">${w(e.currentTime)}</span>
              ...
              <span class="player-time-total" id="player-time-total">${w(e.duration)}</span>
            </div>
          </div>
      ...
      `,(y=document.getElementById("player-track-meta"))==null||y.addEventListener("click",C=>{C.target.closest("#btn-player-fav")||o.setSelectedHymn(a)}),(I=document.getElementById("btn-player-fav"))==null||I.addEventListener("click",C=>{C.stopPropagation(),o.toggleFavorite(a.number)}),(L=document.getElementById("btn-player-main-play"))==null||L.addEventListener("click",()=>{E.getState().isPlaying?E.pause():E.resume()}),(k=document.getElementById("btn-player-prev"))==null||k.addEventListener("click",()=>{E.playPrevious()}),(T=document.getElementById("btn-player-next"))==null||T.addEventListener("click",()=>{E.playNext()}),(M=document.getElementById("btn-player-close"))==null||M.addEventListener("click",()=>{E.stop()});const S=document.getElementById("player-timeline-slider");S==null||S.addEventListener("mousedown",()=>{this.isUserSeeking=!0}),S==null||S.addEventListener("touchstart",()=>{this.isUserSeeking=!0}),S==null||S.addEventListener("change",C=>{this.isUserSeeking=!1,E.seekTo(parseFloat(C.target.value))}),S==null||S.addEventListener("input",C=>{const x=document.getElementById("player-time-current");x&&(x.textContent=w(parseFloat(C.target.value)))});return}const c=document.getElementById("player-mobile-progress-fill");c&&(c.style.width=`${n}%`);const u=document.getElementById("player-main-play-icon");u&&(u.innerHTML=e.isPlaying?d.pause:d.play);const p=document.getElementById("btn-player-fav");p&&(p.classList.toggle("is-fav",i),p.innerHTML=i?d.heart:d.heartOutline);const h=document.getElementById("btn-player-prev");h&&(h.disabled=!e.hasPrevious);const m=document.getElementById("btn-player-next");m&&(m.disabled=!e.hasNext);const g=document.getElementById("player-time-current");g&&(g.textContent=w(e.currentTime));const v=document.getElementById("player-time-total");v&&(v.textContent=w(e.duration));const f=document.getElementById("player-timeline-slider");if(f&&(f.max=Math.floor(e.duration),this.isUserSeeking||(f.value=Math.floor(e.currentTime))),s.selectedHymn&&s.selectedHymn.number===a.number){const S=document.getElementById("detail-time-current");S&&(S.textContent=w(e.currentTime));const C=document.getElementById("detail-time-duration");C&&(C.textContent=w(e.duration));const x=document.getElementById("detail-seekbar");x&&!this.isUserSeeking&&(x.max=Math.floor(e.duration),x.value=Math.floor(e.currentTime))}}renderModal(){const t=document.getElementById("modal-host");if(!t)return;const e=o.get();if(!e.activeModal){this.currentActiveModal!==null&&(t.innerHTML="",this.currentActiveModal=null);return}if(this.currentActiveModal!==e.activeModal)switch(this.currentActiveModal=e.activeModal,e.activeModal){case"PROFILE":this.renderProfileModal(t);break;case"CREATE_PLAYLIST":this.renderCreatePlaylistModal(t);break;case"ADD_TO_PLAYLIST":this.renderAddToPlaylistModal(t,e.modalPayload);break;case"DELETE_PLAYLIST":this.renderDeletePlaylistModal(t,e.modalPayload);break;default:t.innerHTML="",this.currentActiveModal=null}}renderProfileModal(t){var c,u,p,h,m,g;const e=o.get(),a=e.userProfile,s=[{hex:"#14C69B",label:"Verde"},{hex:"#006680",label:"Azul"},{hex:"#DCBB11",label:"Oro"},{hex:"#8E24AA",label:"Púrpura"},{hex:"#E53935",label:"Rojo"},{hex:"#3949AB",label:"Índigo"}];t.innerHTML=`
    ...
    `,(c=document.getElementById("btn-close-modal"))==null||c.addEventListener("click",()=>o.closeModal()),(u=document.getElementById("btn-cancel-profile"))==null||u.addEventListener("click",()=>o.closeModal()),(p=document.getElementById("modal-overlay"))==null||p.addEventListener("click",v=>{v.target.id==="modal-overlay"&&o.closeModal()});const i=document.getElementById("avatar-box"),n=document.getElementById("avatar-file-input");i==null||i.addEventListener("click",()=>n==null?void 0:n.click()),n==null||n.addEventListener("change",v=>{const f=v.target.files[0];if(f){const y=new FileReader;y.onload=I=>{const L=I.target.result;a.profileImage=L,i.innerHTML=`<img src="${L}" alt="Avatar"><div class="avatar-upload-overlay">${d.camera}</div>`},y.readAsDataURL(f)}}),t.querySelectorAll(".color-option").forEach(v=>{v.addEventListener("click",()=>{t.querySelectorAll(".color-option").forEach(y=>y.classList.remove("selected")),v.classList.add("selected");const f=v.getAttribute("data-color");a.avatarColor=f,i.style.backgroundColor=f})}),t.querySelectorAll("[data-scale]").forEach(v=>{v.addEventListener("click",()=>{t.querySelectorAll("[data-scale]").forEach(y=>y.classList.remove("active")),v.classList.add("active");const f=v.getAttribute("data-scale");o.applyTextScale(f)})}),t.querySelectorAll("[data-theme]").forEach(v=>{v.addEventListener("click",()=>{t.querySelectorAll("[data-theme]").forEach(y=>y.classList.remove("active")),v.classList.add("active");const f=v.getAttribute("data-theme");o.applyTheme(f)})}),(h=document.getElementById("btn-export-backup"))==null||h.addEventListener("click",()=>{const v=b.exportBackup();$(v?"Copia de seguridad descargada":"Error al exportar copia de seguridad")});const l=document.getElementById("backup-file-input");(m=document.getElementById("btn-import-backup-trigger"))==null||m.addEventListener("click",()=>{l==null||l.click()}),l==null||l.addEventListener("change",async v=>{const f=v.target.files[0];f&&(await b.importBackup(f)?(o.set({favorites:b.getFavorites(),playlists:b.getPlaylists()}),$("Copia de seguridad importada con éxito"),o.closeModal()):$("Error al importar archivo de copia"))}),(g=document.getElementById("btn-save-profile"))==null||g.addEventListener("click",()=>{var f;const v=((f=document.getElementById("profile-name-input"))==null?void 0:f.value.trim())||"";b.setUserName(v),b.setUserAvatarColor(a.avatarColor),b.setUserProfileImage(a.profileImage),o.set({userProfile:{name:v,avatarColor:a.avatarColor,profileImage:a.profileImage}}),$("Perfil actualizado"),o.closeModal()})}renderCreatePlaylistModal(t){var e,a,s,i;t.innerHTML=`
      <div class="modal-overlay" id="modal-overlay">
        <div class="modal-dialog">
          <div class="modal-header">
    ...
    `,(e=document.getElementById("btn-close-modal"))==null||e.addEventListener("click",()=>o.closeModal()),(a=document.getElementById("btn-cancel-playlist"))==null||a.addEventListener("click",()=>o.closeModal()),(s=document.getElementById("modal-overlay"))==null||s.addEventListener("click",n=>{n.target.id==="modal-overlay"&&o.closeModal()}),(i=document.getElementById("btn-save-playlist"))==null||i.addEventListener("click",()=>{var c;const n=(c=document.getElementById("playlist-name-input"))==null?void 0:c.value.trim();if(!n){$("Ingresa un nombre para la lista");return}b.createPlaylist(n)?(o.refreshPlaylists(),$("Lista creada"),o.closeModal()):$("Ya existe una lista con ese nombre")})}renderAddToPlaylistModal(t,e){var l,c,u,p,h;const a=e==null?void 0:e.hymn;if(!a){o.closeModal();return}const s=()=>{const m=b.getPlaylists();return m.length===0?`
        ...
        <div class="playlist-modal-list custom-scroll">
          ${m.map(g=>{const v=g.hymns.includes(a.number);return`
              <div class="playlist-select-item ${v?"contains-hymn":""}" data-pl-name="${g.name}">
                ...
                <button class="playlist-select-btn ${v?"active":""}" data-action-pl="${g.name}" title="${v?"Quitar de lista":"Añadir a lista"}">
                  ${v?`${d.check} Añadido`:`${d.add} Añadir`}
                </button>
              </div>
            `}).join("")}
        </div>
    ...
    `;const i=()=>{const m=document.getElementById("modal-playlists-host");m&&m.querySelectorAll(".playlist-select-item").forEach(g=>{g.addEventListener("click",()=>{const v=g.getAttribute("data-pl-name"),y=b.getPlaylists().find(L=>L.name.toLowerCase()===v.toLowerCase());y&&y.hymns.includes(a.number)?(b.removeHymnFromPlaylist(v,a.number),$(`Eliminado de "${v}"`)):(b.addHymnToPlaylist(v,a.number),$(`Añadido a "${v}"`)),o.refreshPlaylists(),m.innerHTML=s(),i()})})};i();const n=()=>{const m=document.getElementById("new-playlist-quick-input"),g=m==null?void 0:m.value.trim();if(!g){$("Ingresa un nombre para la lista");return}if(b.createPlaylist(g)){b.addHymnToPlaylist(g,a.number),o.refreshPlaylists(),$("Lista creada y añadido"),m&&(m.value="");const f=document.getElementById("modal-playlists-host");f&&(f.innerHTML=s(),i())}else $("Ya existe una lista con ese nombre")};(l=document.getElementById("btn-quick-create-pl"))==null||l.addEventListener("click",n),(c=document.getElementById("new-playlist-quick-input"))==null||c.addEventListener("keydown",m=>{m.key==="Enter"&&(m.preventDefault(),n())}),(u=document.getElementById("btn-close-modal"))==null||u.addEventListener("click",()=>o.closeModal()),(p=document.getElementById("btn-done-add-modal"))==null||p.addEventListener("click",()=>o.closeModal()),(h=document.getElementById("modal-overlay"))==null||h.addEventListener("click",m=>{m.target.id==="modal-overlay"&&o.closeModal()})}renderDeletePlaylistModal(t,e){var s,i,n,l;const a=e==null?void 0:e.name;if(!a){o.closeModal();return}t.innerHTML=`
      <div class="modal-overlay" id="modal-overlay">
        <div class="modal-dialog">
          <div class="modal-header">
